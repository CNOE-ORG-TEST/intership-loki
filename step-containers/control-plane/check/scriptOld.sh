...
# for generete kubeconfig for secret
...
# Assume new role (limit permission)
ROLE_NAME="cnoe-role-${CLUSTER_NAME}-cp"
ROLE_ARN="arn:aws:iam::${DEPLOY_AWS_ACCOUNT_ID}:role/${ROLE_NAME}"
echo "Assuming role: ${ROLE_ARN}"
aws sts assume-role --role-arn "${ROLE_ARN}" --role-session-name=session-role-controlpanel-$$ --region "${DEPLOY_AWS_REGION}" --duration-seconds 43200
ROLE_ASSUMED="$(aws sts get-caller-identity)"
echo "Role assumed: ${ROLE_ASSUMED}"

# download some config
S3_URL="s3://enel-prod-infr-automation-releases-${DEPLOY_AWS_REGION}/mk8s/controlpanel/release-${BAMBOOENV}/${CONTROLPANEL_VERSION}/aws_infrastructure_packaged.yaml" # cloud formation
...
if [ "${RETURN_CODE}" -ne 0 ] && [[ "${CF}" == *"Stack with id ${CLOUDFORMATION_NAME} does not exist"* ]]; then
  ...
  #CHECK IF ALREADY EXIST THE SECRET (per collegarsi al cluster) -> IF EXIST THERE IS SOME ERROR IN PLAN CONFIGURATION
  ...
  if [ "${RETURN_CODE}" -eq 0 ]; then 
    ...
    if [ "${RETURN_CODE}" -ne 0 ]; then # non esiste il secret
