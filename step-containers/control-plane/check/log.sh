...
function log() {
  ...
  colours['NOTICE']='\033[1;32m'  # Unused
  colours['WARN']='\033[1;33m'      # Yellow
  ...
  local std_line="${colour}[${upper}] ${line}${norm}";
  ...
  case "${level}" in
    ...
    'error')
      ...
      #Launching error cleanup command on error
      #cleanup_dir
...
#Assing permission to Bamboo to clean the build dir
cleanup_dir(){
    echo "Assing Bamboo permission to cleanup"
    #Add permissions for bamboo user
    if [ -z ${USERID} ]; then #fix for retrocompatibility
      echo "Skipping chown command"
    else
      echo "Adding bamboo permissions to /data dir"
...
#Print line number on 'ERR' signal
trap 'err_report $LINENO' ERR

#Execute cleanup operation 'EXIT' signal
