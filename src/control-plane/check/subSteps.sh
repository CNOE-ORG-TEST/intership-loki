...
function checkControlpanelVsInfrpanel () {
  ...
  local INFRPANEL_K8S_VERSION="$(kubectl get cm cm-version -n kube-system -o "jsonpath={.data.version}")"
  local RETURN_CODE=$?
  set -e
  if [ "${RETURN_CODE}" -eq 0 ]; then
    ...
    >&2 colorEcho "error" "Control of compatibility between controlplane version and  infrpanel version not possible, the config map cm-version isn't present on cluster ${1}."
    exit 1
  fi
}
...
function checkControlpanelVsDatapanel () {
    ...
    echo "${GITHUB_TOKEN}"
    echo "${GITHUB_ORG}/${1}Dataplane"
    echo "$(repoExist "${GITHUB_ORG}/${1}Dataplane" "${GITHUB_TOKEN}")"
