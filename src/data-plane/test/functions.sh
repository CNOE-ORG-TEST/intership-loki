    ...
    if [ "${REPLICAS_AVAILABLE}" = "null" ] || [ "${REPLICAS_AVAILABLE}" = "" ]; then
      ...
      if [ "${REPLICAS_DESIRED}" != "${REPLICAS_AVAILABLE}" ]; then
        ...
        for LOCAL_RETRIES in {1..10}; do
            ...
            echo -e "The deployment ${DEPLOY_NAME} is not healty.\nAttending 60 seconds and checks it again."
            sleep 60
        done
...
function ExistInfrplane(){
  set +e
  local INFRPANEL_K8S_VERSION="$(kubectl get cm cm-version -n kube-system -o "jsonpath={.data.version}" 2>&1)"
  local RETURN_CODE=$?
  set -e
...
# $1 : name of the plugin to check
function checkEnablePlugin(){
  if [ "$(kubectl get cm cm-version -n kube-system -o "jsonpath={.data.${1}}")" = "true" ]; then
    echo "true"
  else
    echo "false"
  fi
